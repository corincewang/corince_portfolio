  ...
  return (
    <div className="space-y-8 grid-container">
      {illustrations.map((illustration) => (
        <article
          key={illustration.id}
          className="group bg-white border border-gray-200 hover:border-gray-300 transition-all duration-300 p-8 grid-item hover-lift"
        >
          <div className="flex items-start justify-between mb-4">
            <div className="flex items-center space-x-4">
              <div className="flex items-center space-x-1 text-sm text-gray-500">
                <Calendar size={16} />
                <span>{new Date(illustration.date).toLocaleDateString('en-US', { 
                  year: 'numeric', 
                  month: 'long', 
                  day: 'numeric' 
                })}</span>
              ...
              <div className="flex items-center space-x-1 text-sm text-gray-500">
                <Clock size={16} />
                <span>{illustration.readTime}</span>
              </div>
            </div>
